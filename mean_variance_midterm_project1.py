...
def _parse_date_series(date_series: pd.Series) -> pd.Series:
    """
    Robust date parser for common finance CSV formats.

    Handles ISO dates (YYYY-MM-DD), European-style dates (DD-MM-YYYY),
    U.S.-style dates (MM-DD-YYYY), and mixed files. It tries multiple parsing
    conventions and keeps the version that successfully parses the most rows.
    """
    ...
    if best.notna().sum() == 0:
        raise ValueError(
            "Could not parse the Date column. Please make sure the CSV uses a standard date format "
            "such as YYYY-MM-DD or DD-MM-YYYY."
        )
    ...
    if len(bad_rows) > 0:
        ...
        raise ValueError(
            f"Some dates could not be parsed: {sample}. "
            "Please clean the Date column or convert it to YYYY-MM-DD."
        )
